...
const dashboardService = {
    ...
    getParentDashboard: async (student_id, campus_id) => {
        ...
        // ✅ FIXED: Join with attendance_status table
        const [attendance] = await db.execute(
            `SELECT 
                ...
                ROUND((SUM(CASE WHEN ast.status = 'Present' THEN 1 ELSE 0 END) / COUNT(*)) * 100, 2) as percentage
            FROM attendance a
        ...
        const [upcomingEvents] = await db.execute(
            `SELECT 'Exam' as type, e.name as title, e.exam_date as event_date
            FROM exams e
            JOIN classes c ON e.class_id = c.id
            JOIN students s ON s.class_name = c.name
            WHERE s.id = ? AND e.exam_date >= CURDATE()
            ORDER BY e.exam_date ASC
            LIMIT 5`,
            [student_id]
        );
